...
class SearchActivity : AppCompatActivity(), ReferCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        iv_search.setOnClickListener {
            if(et_search.text.toString().length >= 3){
                progress.show()
                searchForCompany(et_search.text.toString())
            }
        }
    ...
    private fun searchForCompany(query: String) {
        ...
        profileAdapter.notifyDataSetChanged()
        if (uid != null) {
            apiService.search(query, uid).enqueue(object : Callback<ListResponseModel> {
    ...
    //                        candidatesItem.add(GetCandidatesItem("", "", it.ref_gid, it.referrer_name, it.company_logo, 192, "", it.company_linkedin, it.company_logo, it.company_name))
                        ...
                        try {
                            ...
                            Toast.makeText(this@SearchActivity, errorResponse.message, Toast.LENGTH_SHORT)
                                .show()
                        } catch (e: Exception) {
                            e.printStackTrace()
                        }
